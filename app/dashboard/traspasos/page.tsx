...
import {
 ...
  updateTraspaso, // Asegúrate de importar updateTraspaso
  type AddTraspasoData,
  type KardexMovement,
  type KardexRequestInfo, 
} from '@/lib/traspasos-store';
import { getProducts } from '@/lib/product-store'; // Asumiendo que existe
...
import EditTraspasoModal from '@/components/traspasos/EditTraspasoModal'; // Importa el modal de edición
...
export default function TraspasosPage() {
 ...
  }, [token]);
  
  useEffect(() => { fetchTraspasos(); }, [fetchTraspasos]);
 ...
    if (!token) return;
    const confirmMsg = status === 'APPROVED' ? "¿Aprobar este traspaso?" : "¿Rechazar este traspaso?";
 ...
    await fetchTraspasos(); // Refresca la data
  };
 ...
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-3xl font-bold text-slate-900 dark:text-white">Gestión de Traspasos</h1>
          <p className="mt-1 text-slate-600 dark:text-slate-400">Movimientos de inventario entre almacenes.</p>
        </div>
        <Button onClick={() => setIsAddModalOpen(true)} className="flex items-center gap-2" disabled={almacenes.length === 0 || productos.length === 0}>
          <Plus className="h-4 w-4" /> Solicitar Traspaso
        </Button>
      </div>
      
      {error && <p className="p-4 text-center text-red-500 bg-red-100 dark:bg-red-900/50 rounded-lg">{error}</p>}
