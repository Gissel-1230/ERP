...
export default function DashboardLayout({
  ...
  const handleLogout = () => {
    ...
    document.cookie = 'token=; path=/; expires=Thu, 01 Jan 1970 00:00:00 GMT'; // Elimina la cookie del token
    router.push('/');
  }
  ...
  useEffect(() => {
    if (!user) {
      router.push('/'); // si no hay sesión, redirige al login
    }
  }, [user]);
    ...
    <div className="flex h-screen w-full bg-slate-50 dark:bg-slate-900">
      ...
      <aside className="hidden w-64 flex-col border-r bg-slate-800 text-white sm:flex">
        ...
        <div className="border-t border-slate-700 p-4">
          ...
          <button onClick={handleLogout} className="mt-2 flex w-full items-center gap-3 rounded-md px-3 py-2 text-left text-slate-300 transition-colors hover:bg-slate-700 hover:text-white">
