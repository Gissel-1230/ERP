...
const userDataString = localStorage.getItem('user'); // clave "user"
  if (userDataString) {
    try {
      const userData = JSON.parse(userDataString);
      if (userData && userData.full_name) {
        setUserName(userData.full_name);
        setUserRole(userData.role_id || null);
      }
    } catch (e) {
    ...
    setUserName('anonimo');
      ...
      //asegurar la existencia de full_name y role_id
      if(userData){
        setUserName(userData.full_name || '');
        setUserRole(userData.role_id || null);//guardamo el rol
      }
    }
  }, []);
    ...
    <div className="flex flex-col gap-8">
      ...
      <div>
        <h1 className="text-3xl font-bold text-slate-900 dark:text-white">
          Bienvenida de nuevo, {userName || 'anonimo'}
        </h1>
