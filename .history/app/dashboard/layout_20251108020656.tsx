...
const navItems = [
  ...
  { href: "/dashboard/requisicion", icon: ClipboardList, label: "Requisición", roles: [1, 2] },
];
...
// Filtrar solo rutas permitidas al rol
...
return (
  <div className="flex h-screen w-full bg-slate-50 dark:bg-slate-900">
    ...
    <aside className="hidden w-64 flex-col border-r bg-slate-800 text-white sm:flex">
      ...
      {/* Configuración: solo para Director y Subdirector */}
      ...
      {/* Para Usuario General solo mostrar logout */}
      {currentUser.role_id === 3 && (
        <div className="border-t border-slate-700 p-4">
          <button
            onClick={handleLogout}
            className="flex w-full items-center gap-3 rounded-md px-3 py-2 text-left text-slate-300 transition-colors hover:bg-slate-700 hover:text-white"
          >
            <LogOut className="h-5 w-5" />
            <span>Cerrar Sesión</span>
          </button>
