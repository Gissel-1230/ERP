...
export default function AlmacenesPage() {
  ...
  const handleDelete = async (id: string) => {
    ...
    if (window.confirm('¿Estás seguro de que quieres eliminar este almacén?')) {
      ...
      // Opcional: Podrías activar un estado 'isDeleting' aquí para mostrar feedback
      
      try {
        // 2. Llama a la API para eliminar
        await deleteAlmacenById(id, token);

        // 3. Si tiene éxito (no lanzó error), refresca la lista INMEDIATAMENTE
        console.log(`Almacén ${id} eliminado (según la API), refrescando lista...`);
        if (token) await refreshAlmacenes(token); 

      } catch (err: any) {
          console.error("Error al eliminar almacén:", err);
          
          // 4. MUESTRA EL ERROR QUE VINO DEL BACKEND (ej. "No se puede eliminar...")
          setError(err.message || "No se pudo eliminar el almacén.");

          // 5. TEMPORIZADOR PARA LIMPIAR EL ERROR Y REFRESCAR
          setTimeout(() => {
              console.log("Temporizador: Limpiando error y refrescando lista...");
              setError(null); // Limpia el mensaje de error
              // Opcional: Volver a cargar la lista por si algo cambió mientras se mostraba el error
              // if (token) refreshAlmacenes(token); 
          }, 5000); // 5000 milisegundos = 5 segundos
          // --- FIN DE LA MODIFICACIÓN ---

      } finally {
          // Opcional: Desactivar 'isDeleting' si lo usaste
      }
    }
  };
