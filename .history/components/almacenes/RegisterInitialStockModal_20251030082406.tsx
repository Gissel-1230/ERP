...
export default function RegisterInitialStockModal({ 
    isOpen, onClose, onSave, warehouseId, isSaving, saveError 
    ...
    const filteredProducts = products.filter(p => {
        ...
        const numericCategoryId = String(selectedCategoryId).split('-')[1] || selectedCategoryId;
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        onSave({
            ...
            description: description || `Entrada inicial de stock al Almacén ${rawWarehouseId}.`,
            movement_type: 'IN',
        });
    };
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm" onClick={onClose}>
            <div className="relative w-full max-w-lg rounded-xl bg-white p-6 shadow-xl dark:bg-slate-800" onClick={(e) => e.stopPropagation()}>
                <h2 className="text-xl font-bold text-slate-900 dark:text-white">Registrar Stock Inicial</h2>
                ...
                <form onSubmit={handleSubmit} className="mt-6 space-y-4">
                    ...
                    <div>
                        ...
                        >
                            ...
                            {!isLoadingSelects && <option value="" disabled>Seleccionar Categoría</option>}
                            {categories.map(cat => (
                    ...
                    <div>
                        ...
                        <select 
                            ...
                            disabled={isLoadingSelects || isSaving || !selectedCategoryId || filteredProducts.length === 0}
                            required
                            className="mt-1 w-full rounded-lg border-slate-300 dark:bg-slate-700 disabled:opacity-50"
                        >
                        ...
                        {filteredProducts.length === 0 && !isLoadingSelects && selectedCategoryId && (
                            <p className="text-xs text-orange-500 mt-1">No hay productos en esta categoría.</p>
                        )}
                    </div>
