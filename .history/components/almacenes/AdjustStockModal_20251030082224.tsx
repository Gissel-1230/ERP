...
export default function AdjustStockModal({
  ...
  const [movementType, setMovementType] = useState<"ADJUST-IN" | "ADJUST-OUT">(
    "ADJUST-IN"
  );
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const rawProductId =
      String(productToAdjust.id).split("-")[1] || productToAdjust.id;
    ...
    >
      ...
      >
        <h2 className="text-xl font-bold text-slate-900 dark:text-white">
          Ajuste de Stock: {productToAdjust.nombre}
        </h2>
        <p className="text-sm text-slate-500 mb-4">
          Stock Actual: {productToAdjust.cantidad}
        </p>
        ...
        <form onSubmit={handleSubmit} className="mt-6 space-y-4">
          ...
          <div>
            ...
            <select
              ...
              value={movementType}
              onChange={(e) =>
                setMovementType(e.target.value as "ADJUST-IN" | "ADJUST-OUT")
              }
              disabled={isSaving}
